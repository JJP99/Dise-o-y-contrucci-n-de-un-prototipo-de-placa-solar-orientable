...
for i=5:5:80
    fichero = sprintf('Encoder_C%d.txt', i);
    load(fichero);
    ppds(:, (i/5)) = eval(sprintf('Encoder_C%d', i));
    % ppds(2:end,(i/5)) = diff(ppds(:,(i/5)))
end
...
rps = (10 * ppds) / (CPR * reductora); % * timer);
...
ylabel('Velocidad Angular (rad/s)', 'FontSize', 18);
legend('Datos Experimentales', 'Simulación del Modelo', 'FontSize', 14);
title('Comparación entre Datos Experimentales y Simulación del Modelo para OCR0B = 5', 'FontSize', 20);
...
xlabel('Tiempo (s)', 'FontSize', 18);
ylabel('Velocidad Angular (rad/s)', 'FontSize', 18);
legend('Datos Experimentales Ajustados', 'Simulación del Modelo', 'FontSize', 14);
title('Comparación entre Datos Experimentales Ajustados y Simulación del Modelo para OCR0B = 20', 'FontSize', 20);


% Función de transferencia OCR0B = 5
tf5 = tf(sys{1});
